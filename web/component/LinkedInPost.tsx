import { align, col, justify, row } from "@/css/layout.css"
import { avatar, card, color, font } from "@/css/style.css"
import { css, cx } from "@/styled-system/css"
import Image from "next/image"

interface Props {
    likes?: number
    comments?: number
    reposts?: number
    children?: React.ReactNode
}

const engageBtn = css({
    display: "flex",
    alignItems: "center",
    gap: "0.4rem",
    padding: "0.25rem 0.4rem",
    borderRadius: "4px",
    fontSize: "0.8125rem",
    fontWeight: 600,
    color: "#666",
    cursor: "pointer",
    _hover: { background: "#f3f2ef" },
    background: "none",
    border: "none",
    fontFamily: "inherit",
})


const menuBtn = css({
    background: "none",
    border: "none",
    cursor: "pointer",
    color: "#666",
    padding: "0.25rem",
    borderRadius: "50%",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    _hover: { background: "#f3f2ef" },
})

const smallAvatar = css({
    width: "24px",
    height: "24px",
    borderRadius: "50%",
    flexShrink: 0,
})

function IconThumbsUp() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16" fill="currentColor">
            <path d="m12.91 7-2.25-2.57a8.2 8.2 0 0 1-1.5-2.55L9 1.37A2.08 2.08 0 0 0 7 0a2.08 2.08 0 0 0-2.06 2.08v1.17a5.8 5.8 0 0 0 .31 1.89l.28.86H2.38A1.47 1.47 0 0 0 1 7.47a1.45 1.45 0 0 0 .64 1.21 1.48 1.48 0 0 0-.37 2.06 1.54 1.54 0 0 0 .62.51h.05a1.6 1.6 0 0 0-.19.71A1.47 1.47 0 0 0 3 13.42v.1A1.46 1.46 0 0 0 4.4 15h4.83a5.6 5.6 0 0 0 2.48-.58l1-.42H14V7zM12 12.11l-1.19.52a3.6 3.6 0 0 1-1.58.37H5.1a.55.55 0 0 1-.53-.4l-.14-.48-.49-.21a.56.56 0 0 1-.34-.6l.09-.56-.42-.42a.56.56 0 0 1-.09-.68L3.55 9l-.4-.61A.28.28 0 0 1 3.3 8h5L7.14 4.51a4.2 4.2 0 0 1-.2-1.26V2.08A.09.09 0 0 1 7 2a.1.1 0 0 1 .08 0l.18.51a10 10 0 0 0 1.9 3.24l2.84 3z" />
        </svg>
    )
}

function IconComment() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16" fill="currentColor">
            <path d="M5 8h5v1H5zm11-.5v.08a6 6 0 0 1-2.75 5L8 16v-3H5.5A5.51 5.51 0 0 1 0 7.5 5.62 5.62 0 0 1 5.74 2h4.76A5.5 5.5 0 0 1 16 7.5m-2 0A3.5 3.5 0 0 0 10.5 4H5.74A3.62 3.62 0 0 0 2 7.5 3.53 3.53 0 0 0 5.5 11H10v1.33l2.17-1.39A4 4 0 0 0 14 7.58zM5 7h6V6H5z" />
        </svg>
    )
}

function IconRepost() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16" fill="currentColor">
            <path d="M4 10H2V5c0-1.66 1.34-3 3-3h3.85L7.42 0h2.44L12 3 9.86 6H7.42l1.43-2H5c-.55 0-1 .45-1 1zm8-4v5c0 .55-.45 1-1 1H7.15l1.43-2H6.14L4 13l2.14 3h2.44l-1.43-2H11c1.66 0 3-1.34 3-3V6z" />
        </svg>
    )
}

function IconSend() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16" fill="currentColor">
            <path d="M14 2 0 6.67l5 2.64 5.67-3.98L6.7 11l2.63 5z" />
        </svg>
    )
}

function IconGlobe() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="12" height="12" fill="currentColor">
            <path d="M8 0a8 8 0 100 16A8 8 0 008 0zM6.06 2.27A6.5 6.5 0 001.56 8H4.1a12.8 12.8 0 011.96-5.73zM4.1 9.5H1.56a6.5 6.5 0 004.5 5.73A12.8 12.8 0 014.1 9.5zm1.9 0a11.29 11.29 0 001.5 4.87A11.29 11.29 0 009 9.5H6zm4.5 0a12.8 12.8 0 01-1.96 5.73A6.5 6.5 0 0014.44 9.5H10.5zm3.94-1.5a6.5 6.5 0 00-4.5-5.73A12.8 12.8 0 0111.9 8h2.54zM10.5 8H9A11.29 11.29 0 007.5 3.13 11.29 11.29 0 006 8h4.5z" />
        </svg>
    )
}

function IconLinkedIn() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="14" height="14">
            <rect width="16" height="16" rx="3" fill="#0a66c2" />
            <path d="M4 6h2v6H4zm1-1.5a1 1 0 1 1 0-2 1 1 0 0 1 0 2zM7 6h2v.85A2.5 2.5 0 0 1 11.5 6C13 6 13 7.5 13 8.5V12h-2V9c0-.75-.25-1.5-1-1.5S9 8 9 9v3H7z" fill="white" />
        </svg>
    )
}

function IconMoreHoriz() {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
            <path d="M6 12a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm8 0a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm8 0a2 2 0 1 1-4 0 2 2 0 0 1 4 0z" />
        </svg>
    )
}

export default function LinkedInPost({ likes, comments, reposts, children }: Props) {
    return (
        <div className={card}>
            {/* Header */}
            <div className={cx(row, align.start)} style={{ gap: "0.75rem" }}>
                <Image
                    src="/gilad.jpeg"
                    alt="Gilad Kutiel"
                    width={48}
                    height={48}
                    className={avatar}
                />
                <div className={cx(col)} style={{ gap: "0.1rem", flex: 1 }}>
                    <div className={cx(row, align.center)} style={{ gap: "0.3rem" }}>
                        <span className={cx(font.weight.bold, font.size.base)}>Gilad Kutiel</span>
                        <IconLinkedIn />
                        <span className={cx(font.size.sm, color.muted)}>• You</span>
                    </div>
                    <div className={cx(font.size.sm, color.muted)}>
                        PhD | Machine Learning | ex-Google, Facebook, Amazon
                    </div>
                    <div className={cx(row, align.center, font.size.sm, color.muted)} style={{ gap: "0.25rem" }}>
                        <span>18h</span>
                        <span>•</span>
                        <IconGlobe />
                    </div>
                </div>
                <button className={menuBtn}>
                    <IconMoreHoriz />
                </button>
            </div>

            {/* Content */}
            <div className={cx(font.size.base)}>{children}</div>

            <div style={{ borderTop: "1px solid #e0e0e0" }} />

            {/* Action bar */}
            <div className={cx(row, align.center, justify.between)}>
                <div className={cx(row, align.center)} style={{ gap: "0.25rem" }}>
                    {/* Viewer avatar + reaction dropdown */}
                    <div className={cx(row, align.center)} style={{ gap: "0.15rem" }}>
                        <Image
                            src="/gilad.jpeg"
                            alt=""
                            width={24}
                            height={24}
                            className={smallAvatar}
                        />
                        <svg viewBox="0 0 10 6" width="10" height="6" fill="#666">
                            <path d="M0 0l5 6 5-6z" />
                        </svg>
                    </div>

                    <button className={engageBtn}>
                        <IconThumbsUp />
                        {likes != null && likes}
                    </button>
                    <button className={engageBtn}>
                        <IconComment />
                        {comments != null && comments}
                    </button>
                    <button className={engageBtn}>
                        <IconRepost />
                        {reposts != null && reposts}
                    </button>
                    <button className={engageBtn}><IconSend /></button>
                </div>
            </div>
        </div>
    )
}
