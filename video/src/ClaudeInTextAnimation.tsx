import { Easing, interpolate, useCurrentFrame, spring, useVideoConfig } from 'remotion'

// Paths extracted from web/public/logo.svg
const BG_PATH =
...
export function ClaudeInTextAnimation() {
  ...
  // Logo bounces in as a whole unit
  const logoScale = spring({ frame, fps, config: { damping: 12, stiffness: 120, mass: 0.8 } })

  // Spark pulses after logo settles (~frame 20)
  const sparkPulse = interpolate(
    frame,
    [22, 35, 48],
    [1, 1.6, 1],
  ...
  // Spark center in SVG space (viewBox "2 2 12 12")
  ...
  return (
    <svg width="800" height="800" viewBox="2 2 12 12" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Whole logo bounces in from center */}
      <g transform={`translate(${CX},${CY}) scale(${logoScale}) translate(${-CX},${-CY})`}>
      ...
      {/* Spark scales up independently for pulse */}
      <g transform={`translate(${SPARK_CX},${SPARK_CY}) scale(${sparkPulse}) translate(${-SPARK_CX},${-SPARK_CY})`}>
        <g transform="translate(3.278,3.292) scale(0.116)">
          <path fill="#D97757" d={SPARK_PATH} />
        </g>
      </g>
