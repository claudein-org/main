...
// from: translate(3.278,3.292) scale(0.116) × local center ≈ (9.5,11)
const SPARK_CX = 4.38, SPARK_CY = 4.57;
const BG_CX = 8, BG_CY = 8;

export function LinkedInLogoAnimation() {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();

  const sp = (delay: number, stiffness = 200, damping = 18) =>
  ...
  const slide = (t0: number, t1: number) =>
    interpolate(frame, [t0, t1], [0.4, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});

  const bgScale = sp(0, 350, 26);
  const nOpacity = fade(12, 24);
  const nSlide = slide(12, 26);
  const iOpacity = fade(18, 30);
  const iSlide = slide(18, 32);
  const sparkScale = sp(28, 200, 13);
  ...
  return (
    <svg width="800" height="800" viewBox="2 2 12 12" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* "n" letterform — slides up */}
      <g opacity={nOpacity} transform={`translate(0,${nSlide})`}>
        <path fill="#FFFFFF" d="M 12.225,12.225 l -1.778,0 L 10.447,9.44 c 0,-0.664 -0.012,-1.519 -0.925,-1.519 c -0.926,0 -1.068,0.724 -1.068,1.47 l 0,2.834 L 6.676,12.225 L 6.676,6.498 l 1.707,0 l 0,0.783 l 0.024,0 c 0.348,-0.594 0.996,-0.95 1.684,-0.925 c 1.802,0 2.135,1.185 2.135,2.728 l -0.000999999999999,3.14 z"/>
      </g>

      {/* "i" stem — slides up, slightly delayed */}
      <g opacity={iOpacity} transform={`translate(0,${iSlide})`}>
        <path fill="#FFFFFF" d="M 5.559,12.225 l -1.78,0 L 3.779,6.498 l 1.78,0 l 0,5.727 z"/>
      </g>

      {/* Claude spark — pops in from center with bounce */}
